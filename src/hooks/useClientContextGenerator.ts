...
/**
 * Mutation-style hook that triggers the `generate-client-context` backend handler
 * (Gemini-powered identity-guide writer) and reports source counts.
 *
 * NOTE: NOT to be confused with `useClientContext` (TanStack Query data hook in
 * `useClientContext.ts`) — that one reads raw client tables for the viral apps'
 * multi-tenant context loading.
 */
export const useClientContextGenerator = () => {
