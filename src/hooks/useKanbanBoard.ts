...
export function useKanbanBoard() {
  ...
  const { data: columns, isLoading: columnsLoading } = useQuery({
    queryKey: ['kanban-columns', currentWorkspace?.id],
    queryFn: async () => {
  ...
  const { data: cards, isLoading: cardsLoading } = useQuery({
    queryKey: ['kanban-cards', currentWorkspace?.id],
    queryFn: async () => {
      if (!currentWorkspace?.id || !columns || columns.length === 0) return [];
    ...
    enabled: !!currentWorkspace?.id && !!columns && columns.length > 0,
  });
  ...
  const updateCard = useMutation({
    mutationFn: async ({ id, ...updates }: Partial<KanbanCard> & { id: string }) => {
      const { data, error } = await supabase
        .from('kanban_cards')
        .update(updates)
        .eq('id', id)
    ...
    mutationFn: async (cardId: string) => {
      ...
      const { data: content, error: contentError } = await supabase
        ...
        .insert({
          ...
          content_type: card.platform === 'twitter' ? 'tweet' : 
                        card.platform === 'linkedin' ? 'linkedin_post' : 'post',
          metadata: {
            from_kanban: true,
