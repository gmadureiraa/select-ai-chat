...
export function SocialIntegrationsTab({ clientId }: SocialIntegrationsTabProps) {
  ...
  const handleRevalidateTwitter = async () => {
    // Re-validate using stored credentials (just triggers backend validation)
    await validateTwitter.mutateAsync({
      apiKey: "",
      apiSecret: "",
      accessToken: "",
      accessTokenSecret: "",
  ...
  const handleRevalidateLinkedIn = async () => {
    await validateLinkedIn.mutateAsync({ oauthAccessToken: "" });
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-medium">Integrações de Publicação</h3>
        <p className="text-sm text-muted-foreground">
          Configure as APIs para publicação automática nas redes sociais deste cliente
        </p>
      </div>
