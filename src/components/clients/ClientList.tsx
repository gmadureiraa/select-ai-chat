...
export const ClientList = ({ clients, isLoading }: ClientListProps) => {
  ...
  // Fetch content and template counts for each client
  const { data: clientStats } = useQuery({
    queryKey: ["client-stats", clients.map(c => c.id)],
    queryFn: async () => {
      ...
      const stats: Record<string, { contentCount: number; templateCount: number }> = {};
            ...
            <Card key={client.id} className="p-6 border-border/50 bg-card/50 hover:border-border transition-all">
              ...
              <div className="flex items-center gap-4 mb-4 text-xs text-muted-foreground">
                <div className="flex items-center gap-1">
                  ...
                  <span>{stats?.contentCount || 0} conteúdos</span>
                </div>
                <div className="flex items-center gap-1">
                  <FileText className="h-3 w-3" />
                  <span>{stats?.templateCount || 0} templates</span>
                </div>
              </div>
