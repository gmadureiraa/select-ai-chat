...
export interface AttachmentNodeV2Data {
  output?: AttachmentOutput;
  onUpdateData?: (data: Partial<AttachmentNodeV2Data>) => void;
  onDelete?: () => void;
}

const AttachmentNodeV2Component: React.FC<NodeProps<AttachmentNodeV2Data>> = ({ 
  data, 
  selected 
}) => {
    ...
    try {
      if (fileType.startsWith('image/')) {
        ...
        reader.onload = async (event) => {
          ...
          try {
            const { data: analysis, error } = await supabase.functions.invoke('analyze-image', {
        ...
        // Transcribe
        const { data: transcription, error: transcribeError } = await supabase.functions.invoke('transcribe-video', {
...
export const AttachmentNodeV2 = memo(AttachmentNodeV2Component);
