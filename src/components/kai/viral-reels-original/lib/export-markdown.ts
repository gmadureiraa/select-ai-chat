/**
 * Port literal de code/reels-viral/lib/export-markdown.ts.
 *
 * Converte um AdaptResponse em markdown completo pra download/copy.
 * Adaptação: fallback `source ?? sourceMeta` pra suportar shape do KAI.
 */
...
export function buildMarkdown(data: AdaptResponse): string {
  const source = data.source ?? data.sourceMeta ?? {};
  ...
  if (source.url) lines.push(`- **Link:** ${source.url}`);
...
export function downloadMarkdown(data: AdaptResponse, filename?: string): void {
  ...
  const safeTitle = data.script.titulo
    ...
    .slice(0, 60);
  const sourceShort =
    data.source?.shortCode ?? data.sourceMeta?.shortCode ?? "reel";
  const name = filename ?? `roteiro-${safeTitle || sourceShort}.md`;
