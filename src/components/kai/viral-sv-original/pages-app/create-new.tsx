...
import { normalizeDesignTemplate } from "@sv/lib/carousel-templates";

// KAI multi-tenant integration:
// - useSVClient: pega clientId/client passados pelo Kai.tsx via context
// - useClientWorkspaceContext: tom/pillars/persona/refs do cliente
// - useViralContext: cross-app bridge (consume pendingBriefing do Radar/Reels)
// - ClientReferencesPanel: cards de refs visuais + library + concorrentes
...
export default function NewCarouselPage() {
  ...
  // ── KAI client context ──────────────────────────────────────────────
  const { clientId } = useSVClient();
  const { data: clientCtx } = useClientWorkspaceContext(clientId);

  // ── Cross-app bridge: consume pendingBriefing do Radar/Reels ──
  // Carrega só uma vez no mount. SV usa hash routing interno então
  // search params não sobrevivem aos navigates internos do app —
  // por isso preferimos Zustand em vez do `?topic=` legacy (que ainda é
  // suportado mais abaixo no useEffect que lê `searchParams`).
  ...
  useEffect(() => {
    ...
    if (!pending || pending.source === "sv") {
      bridgeAppliedRef.current = true;
      return;
    }
    bridgeAppliedRef.current = true;
    const briefingChunks: string[] = [];
    if (pending.topic) briefingChunks.push(`Tema: ${pending.topic}`);
    if (pending.briefing) briefingChunks.push(pending.briefing);
    if (pending.url) briefingChunks.push(`Fonte: ${pending.url}`);
    const merged = briefingChunks.join("\n\n");
    if (merged) {
      // Coloca o brief no idea state. setIdea é declarado mais abaixo
      // mas a closure hoist resolve OK porque rodamos no useEffect.
      setIdea((current) => current || merged.slice(0, 4900));
      const sourceLabel =
        pending.source === "radar" ? "Radar Viral" : "Reels Viral";
  ...
  // ── Auto-save por cliente: rascunho da idea em localStorage (Fase G) ──
  // Centralizado via `useViralAutoSave`. Key namespacing pra não cruzar
  // drafts entre clientes. A hidratação acontece num useEffect mais abaixo
  // (precisa do `setIdea` declarado adiante).
  const draftKey = clientId ? `kai-viral-sv-draft-${clientId}` : "";
  ...
  // Hidrata tone do CLIENTE Kaleidos quando disponível — sobrescreve o do
  // profile (do user solo) porque carrossel é PARA o cliente, não pro user.
  // Mapeia preference_value (texto livre cadastrado no onboarding) pro
  // enum local — se não bater, mantém o tone atual.
  const clientToneAppliedRef = useRef(false);
  useEffect(() => {
    if (clientToneAppliedRef.current) return;
    if (!clientCtx?.tone) return;
    const raw = clientCtx.tone.toLowerCase().trim();
    const mapped: Tone | null =
      raw.includes("editor")
        ? "editorial"
        : raw.includes("inform") || raw.includes("casual")
          ? "informal"
          : raw.includes("direto") || raw.includes("data")
            ? "direto"
            : raw.includes("provoc") || raw.includes("hot")
              ? "provocativo"
              : null;
    if (mapped) {
      setTone(mapped);
      clientToneAppliedRef.current = true;
    }
  }, [clientCtx]);
  ...
  // Auto-save via useViralAutoSave (Fase G). Persiste a idea atual com
  // debounce 1s e expõe status pro `<AutoSaveIndicator />`. shouldPersist
  // exige idea com >= 8 chars pra não poluir storage com lixo.
  const draftData = useMemo(() => ({ idea }), [idea]);
  const autoSave = useViralAutoSave({
    key: draftKey || "kai-viral-sv-draft-noop",
    data: draftData,
    enabled: !!draftKey,
    shouldPersist: (d) => d.idea.trim().length >= 8,
  });
  // Hidrata o draft uma vez por cliente via restore().
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Antes do navigate, limpa draft do cliente — geração concluída.
      autoSave.clear();
    ...
    >
      ...
      <div className="mt-5" style={{ minWidth: 0 }}>
        <div className="flex flex-col gap-4" style={{ minWidth: 0 }}>
          ...
          {/* KAI client refs — refs visuais + top library + concorrentes */}
          {clientCtx?.client && (
