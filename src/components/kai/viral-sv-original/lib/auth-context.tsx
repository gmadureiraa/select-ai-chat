...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    // KAI bridge: profile bruto + workspace_subscriptions → mapeado pra
    // shape `UserProfile` legacy do SV. Apos migration 0008, profiles tem
    // usage_count, usage_limit, sv_plan, sv_period_start, referral_code,
    // referred_by — porem `plan` no SV usa namespace legacy (free/pro/business)
    // que mapeia do `subscription_plans.type` (free/starter/pro/enterprise).
    type ProfileRow = {
      id: string;
      email: string | null;
    ...
    type SubscriptionRow = {
      ...
      subscription_plans: { type: string } | null;
    };

    const profileResp = await (
      ...
      .eq("id", userId)
      .maybeSingle();
    const profileRow = (profileResp.data as ProfileRow | null) ?? null;

    // Pega workspace_id (primeira membership) e tenta buscar subscription.
    let subscriptionPlanType: string | null = null;
    let subscriptionPeriodStart: string | null = null;
    try {
      const membResp = await (
        supabase as unknown as {
          from: (t: string) => {
            select: (s: string) => {
              eq: (k: string, v: string) => {
        ...
        .limit(1)
        .maybeSingle();
      const workspaceId = (membResp.data as { workspace_id?: string } | null)
        ?.workspace_id;
      if (workspaceId) {
        const subResp = await (
          supabase as unknown as {
          ...
          .select(
            "status, current_period_start, subscription_plans(type)",
          )
          .eq("workspace_id", workspaceId)
          .maybeSingle();
        const sub = subResp.data as SubscriptionRow | null;
        subscriptionPlanType = sub?.subscription_plans?.type ?? null;
        subscriptionPeriodStart = sub?.current_period_start ?? null;
      }
    } catch (err) {
      console.warn("[fetchProfile] workspace/subscription lookup failed:", err);
    }

    if (!profileRow) return null;

    // Mapeamento KAI plan → SV legacy plan. Quotas inferidas do plano:
    //   free/starter → free (5/mes), pro → pro (30/mes), enterprise → business (∞)
    const planType = subscriptionPlanType ?? profileRow.sv_plan ?? "free";
    let svPlan: "free" | "pro" | "business" = "free";
    let svUsageLimit = 5;
    if (planType === "pro") {
      svPlan = "pro";
      svUsageLimit = 30;
    ...
    const mapped: UserProfile = {
      ...
      carousel_style: "",
      plan: svPlan,
      usage_count: profileRow.usage_count ?? 0,
      // Override manual (Gabriel deu bonus) supera quota do plano.
      usage_limit:
        profileRow.usage_limit && profileRow.usage_limit > svUsageLimit
          ? profileRow.usage_limit
          : svUsageLimit,
      onboarding_completed: true,
      // Campos extras do SV (referral, etc) — projetados em runtime via
      // cast pra que o consumer receba os valores reais quando precisar.
    };
    // Anexa campos nao-tipados em `UserProfile` mas usados em runtime.
    (mapped as UserProfile & {
      period_start?: string;
      referral_code?: string | null;
      referred_by?: string | null;
    }).period_start = subscriptionPeriodStart ?? profileRow.sv_period_start ?? undefined;
    (mapped as UserProfile & {
      referral_code?: string | null;
      referred_by?: string | null;
    }).referral_code = profileRow.referral_code ?? null;
    (mapped as UserProfile & {
      referred_by?: string | null;
    }).referred_by = profileRow.referred_by ?? null;
  ...
  const updateProfile = useCallback(
    async (data: Partial<UserProfile>) => {
      ...
      if (updated) setProfile(updated as unknown as UserProfile);
    },
    [user]
  );
