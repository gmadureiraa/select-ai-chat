...
import { Image as ImageIcon, Upload, Loader2, Save, Link, Users, MousePointer, Eye, ExternalLink } from "lucide-react";
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
            ...
            <div className="flex-1 space-y-3">
              <div>
                ...
                <div className="flex gap-2">
                  <Input value={formData.permalink} onChange={(e) => updateField("permalink", e.target.value)} placeholder="https://instagram.com/p/..." className="h-8 text-sm" />
                  {formData.permalink && (
                    <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" asChild>
                      <a href={formData.permalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-3.5 w-3.5" /></a>
                    </Button>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="date" value={formData.posted_at} onChange={(e) => updateField("posted_at", e.target.value)} className="h-8 text-sm" />
                </div>
                <div>
          ...
          <div>
            ...
            <Textarea value={formData.caption} onChange={(e) => updateField("caption", e.target.value)} placeholder="Legenda do post..." className="min-h-[80px] text-sm resize-none" />
          </div>

          <Separator />

          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2"><Eye className="h-4 w-4 text-blue-500" />Métricas de Engajamento</h4>
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={metric.key}>
                  ...
                  <Input type="number" value={formData[metric.key as keyof typeof formData] as number} onChange={(e) => updateField(metric.key, e.target.value)} className="h-8 text-sm" min={0} step={metric.key === "engagement_rate" ? "0.01" : "1"} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2"><MousePointer className="h-4 w-4 text-green-500" />Métricas de Conversão</h4>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label className="text-[10px] text-muted-foreground flex items-center gap-1"><Link className="h-3 w-3" /> Cliques no Link</Label>
                <Input type="number" value={formData.link_clicks} onChange={(e) => updateField("link_clicks", e.target.value)} className="h-8 text-sm" min={0} />
              </div>
              <div>
                <Label className="text-[10px] text-muted-foreground flex items-center gap-1"><Eye className="h-3 w-3" /> Visitas no Perfil</Label>
                <Input type="number" value={formData.profile_visits} onChange={(e) => updateField("profile_visits", e.target.value)} className="h-8 text-sm" min={0} />
              </div>
              <div>
                <Label className="text-[10px] text-muted-foreground flex items-center gap-1"><Users className="h-3 w-3" /> Novos Seguidores</Label>
                <Input type="number" value={formData.follows_from_post} onChange={(e) => updateField("follows_from_post", e.target.value)} className="h-8 text-sm" min={0} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.content_objective} onValueChange={(v) => updateField("content_objective", v)}>
                  <SelectTrigger className="h-9"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                  <SelectContent>
                    {CONTENT_OBJECTIVES.map(obj => <SelectItem key={obj.value} value={obj.value}>{obj.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <Switch checked={formData.is_collab} onCheckedChange={(checked) => updateField("is_collab", checked)} />
              </div>
            </div>
          </div>
        </div>
